...
import patternAudience
from nltk import word_tokenize

class semantic_match:
 regduone = r"(long.*(view|look|see))"
 regdutwo = r"((take|cover|going|finish) .* ((\d(\s)*(-|to|/)(\s)*)*(\d) hours|minutes))"
 regopenhours = r"(open)"
 ...
 def reg_match(question, entity, dbConnect):
  if(re.search(semantic_match.regduone, question, flags=0)):
   ...
   if('-' in question):
    duration = re.search(semantic_match.regdutwo, question, flags=0).group(1)
   else:
    duration = re.search(semantic_match.regdutwo, question, flags=0).group(4)
   if('less' in question):
    duration = '< '+ duration
   if('more' in question):
    duration = '> '+ duration
   print(duration)
   print("Yes, it will be enough to watch")
   ...
   patternAudience.audienceKidsQuery(entity, client)
   ...
   patternAudience.audienceQuery(entity, client)
...
client = MongoDBConnect.mongoConnect().AttractionDetails.AttractionTest

patternAudience.audienceKidsQuery("Old Town Square",client)
while(True):
 #get input
 question= input()
 #get entity
